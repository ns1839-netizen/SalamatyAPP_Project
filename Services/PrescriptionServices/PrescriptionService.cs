...
//            // خط دفاع عالي: سحب قائمة الأدوية المحلية في أول ثانية قبل بدء اتصال الـ AI الطويل تلافياً لقطع الشبكة
...
//            {
//                Console.WriteLine($">>>> Initial DB Load Failed (Will use fallback): {dbLoadEx.Message}");
//            }
...
//                // 3. الفلترة المرنة (Confidence >= 50%)
...
//                    else
//                    {
//                        // 4. خطة الإنقاذ لايف: الدواء مش متسجل محلياً ⬅️ نسأل الـ search_medicine الـ Endpoint التانية
...
//                                // قراءة الـ Object المليان داتا الراجع من الـ AI
...
//                                // 🔥 خطوة الحسم: نقله فوراً لقائمة الـ AvailableMedicines ليعرض للمريض لايف حالا بالبيانات الراجعة
//                                // بكدة نضمن عرض السعر والوصف حتى لو سيرفر قاعدة البيانات أونلاين فصل شبكة!
...
//                                // محاولة حفظ الدواء في الخلفية كـ Cache (لو نجحت تمام، لو ضربت بسبب الشبكة الأبلكيشن مش هيعلق)
//                                try
//                                {
...
//                                    detectedMed.Id = newProduct.Id; // تحديث الـ ID
//                                    _context.Entry(newProduct).State = EntityState.Detached;
//                                }
//                                catch (Exception dbProductEx)
//                                {
//                                    Console.WriteLine($">>>> Background Database Caching Skipped due to network drop: {dbProductEx.Message}");
...
//                // 5. حفظ الهيستوري والروشتات (معزول ومحمي تماماً من كراش الشبكة والـ UserId)
//                try
//                {
...
//                {
//                    Console.WriteLine($">>>> History Save Skipped due to database connection dropped: {dbEx.Message}");
